...
Next.js 16.3 also maintains a marked block in `AGENTS.md` pointing to documentation bundled with the installed Next.js package. Keep that block alongside this starter's instructions. It gives the assistant version-matched local reference material, while Context7 covers the rest of the stack and current documentation queries.

Try this connection check:

> Use Context7 to resolve Better Auth, then look up the Next.js integration for the version in package.json. Tell me which library ID you used.
