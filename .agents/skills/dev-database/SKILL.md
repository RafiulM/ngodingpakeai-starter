...
Read `docs/local-development.md` and `scripts/database.mjs`. The Next.js app runs on the host; `compose.dev.yaml` runs only PostgreSQL. Use npm wrappers so Next.js environment precedence and the saved Compose project name are applied consistently.
